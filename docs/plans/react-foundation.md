...
and renders portals into the `#header` and `#hero-root` slots in `index.html`.
The header slot supplies the semantic header, and HeroSection supplies its section.
The root itself adds no layout around the legacy page.
...
dismissal, spam fields, form submission, skill icons, section reveals, footer year,
and Vercel analytics. It initializes once at module load. The Google analytics and
...
normalize reset. Navigation and hero have their own CSS files. `src/scss/main.scss`
now imports only the styles/helpers still needed by static sections and the dialog.
